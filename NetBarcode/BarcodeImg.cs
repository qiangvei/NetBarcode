...
namespace NetBarcode
{
    public class BarcodeForImg
    {
        ...
        private readonly string _otherText = string.Empty;
        ...
        public BarcodeForImg(string data,string otherData, Type type, bool showLabel, int width, int height, Font labelFont)
        {
            _autoSize = false;
            _data = data;
            _type = type;
            _showLabel = showLabel;
            _otherText = otherData;
            _width = width;
            _height = height;
        ...
        private Image GenerateImage()
        {
                        ...
                        while (pos < _encodedData.Length)
                        {
                            if (_encodedData[pos] == '1')
                            {
                                graphics.DrawLine(pen,
                                    new Point(pos * iBarWidth + shiftAdjustment + halfBarWidth, topLabelAdjustment+_labelFont.Height),
                                    new Point(pos * iBarWidth + shiftAdjustment + halfBarWidth,
                                        _height + topLabelAdjustment + _labelFont.Height));
                            }
                            pos++;
                        }
                    }
            ...
            if (_otherText != string.Empty)
            {
                image = InsertLineText(image);
            }
            return image;
        }
                ...
                using (var g = Graphics.FromImage(image))
                {
                    ...
                    switch (_labelPosition)
                    {
                        case LabelPosition.BottomCenter:
                            labelY = image.Height / 2; ; //image.Height - (_labelFont.Height);
                    ...
                    g.FillRectangle(new SolidBrush(_backgroundColor),
                        new RectangleF(0, labelY, image.Width,image.Height/2+1));

                    //draw datastring under the barcode image
                    g.DrawString(_data, _labelFont, new SolidBrush(_foregroundColor),
                        new RectangleF(0, labelY+ _labelFont.Height, image.Width, _labelFont.Height), stringFormat);

                    if (_otherText != string.Empty)
                    {
                        //怎么换行？
                        stringFormat.Alignment = StringAlignment.Near;
                        g.DrawString(_otherText, _labelFont, new SolidBrush(_foregroundColor),
                            new RectangleF(8*_labelFont.Height/_labelFont.Size, labelY + _labelFont.Height*3, image.Width, _labelFont.Height), stringFormat);
                    }
        ...
        private Image InsertLineText(Image image)
        {
            try
            {
                using (var g = Graphics.FromImage(image))
                {
                    ...
                    {
                        case LabelPosition.BottomCenter:
                            labelY = image.Height / 2 + (_labelFont.Height * 3);
                            stringFormat.Alignment = StringAlignment.Center;
                            break;
                        case LabelPosition.BottomLeft:
                            labelY = image.Height - (_labelFont.Height);
                    ...
                    //color a background color box at the bottom of the barcode to hold the string of data
                    g.FillRectangle(new SolidBrush(_backgroundColor),
                        new RectangleF(0, labelY, image.Width, image.Height/2));
